...
from .base import require_exact_keys, require_int, require_record, require_schema_version, require_string, require_string_list, require_type
...
class MatchedSelection:
    ...
    def from_dict(cls, value: Any, path: str) -> "MatchedSelection":
        ...
        return cls(require_string(record["task_id"], f"{path}.task_id"), partition, require_int(record["matched_order"], f"{path}.matched_order", minimum=0))
...
class EvaluationProtocol:
    ...
    def from_dict(cls, value: Any) -> "EvaluationProtocol":
        ...
        return cls(
            require_schema_version(record["schema_version"], "protocol.schema_version"), require_string(record["protocol_id"], "protocol.protocol_id"), require_string(record["family_id"], "protocol.family_id"),
            require_string(capsules["baseline"], "protocol.capsules.baseline"), require_string(capsules["candidate"], "protocol.capsules.candidate"),
            ...
            tuple(require_string_list(partitions["development"], "protocol.partitions.development")), tuple(require_string_list(partitions["validation"], "protocol.partitions.validation")), tuple(require_string_list(partitions["untouched"], "protocol.partitions.untouched")),
            require_string(optimizer["method"], "protocol.optimizer_disclosure.method"), require_int(optimizer["candidate_count"], "protocol.optimizer_disclosure.candidate_count", minimum=1), tuple(require_string_list(optimizer["rejected_candidate_ids"], "protocol.optimizer_disclosure.rejected_candidate_ids")), VerifyPolicy.from_dict(record["policy"]),
        )
